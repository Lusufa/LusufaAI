...
const Navbar = () => {
  ...
  return (
    ...
    items-center py-3 px-4 sm:px-20 xl:px-32'>
        <img src={assets.logo} alt="logo" className='w-12 sm:w-16 cursor-pointer' onClick={() => navigate('/')} />
        {
          user ? <UserButton /> 
          : 
          (<button onClick={openSignIn} className="flex items-center gap-2 rounded-full text-sm cursor-pointer 
           bg-primary text-black px-6 py-2 shadow-md hover:bg-primary/90 transition">
